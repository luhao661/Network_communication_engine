...
int main()
{
	...
	while (g_bRun)
	{
		...
		double t = tTime.getElapsedTimeInSecond();
...
void SendThread(int id)
{
	...
	//在发送线程中，启动接收线程
	thread RT1(RecvThread, theBegin, theEnd);//格式：函数名  参数
	RT1.detach();
	...
	while (g_bRun)
	{
		for (int i = theBegin; i < theEnd; ++i)
		{
			...
			if (SOCKET_ERROR != pClientArray[i]->SendData(login))
				++SendCnt;
		}
...
void RecvThread(int theBegin, int theEnd)
{
	...
	while (g_bRun)
	{
		for (int i = theBegin; i < theEnd; ++i)
		{
			if (t.getElapsedTimeInSecond() >= 3 && i == theBegin)
				continue;
