...
void CellServer::Start()
{
	...
	//***理解***
	//mem_fn(&CellServer::OnRun)：mem_fn 是 C++ 标准库中的模板函数，用于将
	//成员函数转换为可调用对象。在这里，&CellServer::OnRun 是 CellServer 类
...
bool CellServer::OnRun()
{
	...
	while (isRun())
	{
		...
		int fd_num = select(m_serv_sock + 1, &fdRead, &fdWrite, &fdExp, &timeout);
		if (fd_num == -1)
		{
...
void EasyTcpServer::time4msg()
{
	...
	if (t >= 1.0)
	{
		...
		printf("持续时间<%lf>, 服务端套接字<%d>, \
客户端连接数<%d>, 所有线程每秒处理包数量=%d\n",
t, m_serv_sock,(int)vec_client.size(), static_cast<int>(recvCnt / t));
