...
#include <atomic>//原子操作
#include <map>
#include "CellTask_1.1.hpp"
#include <memory>//智能指针
#include "CellObjectPool.hpp"//使用对象池
...
typedef shared_ptr<DataHead> DataHeadPtr;
typedef shared_ptr<LogInResult> LogInResultPtr;
...
public:
	...
	int SendData(DataHeadPtr pHead);
};
...
typedef shared_ptr<CellServer>CellServerPtr;
...
public:
	//客户端加入事件
	virtual void NEOnNetJoin(ClientSocketPtr& pClient) = 0;//纯虚函数

	//客户端离开事件
	virtual void NEOnNetLeave(ClientSocketPtr& pClient) = 0;

	virtual void NEOnNetMsg(CellServer* pCellServer, ClientSocketPtr& client_sock, DataHead* pHead) = 0;

	//recv事件
	virtual void NERecv(ClientSocketPtr& client_sock) = 0;
};
...
{
	//要发送到的客户端
	ClientSocketPtr m_pclient_sock;
	//要发送的数据指针
	DataHeadPtr m_pHead;

public:
	CellServerMsgToClientTask(ClientSocketPtr pclient_sock, DataHeadPtr pHead)
	{
...
typedef std::shared_ptr<CellServerMsgToClientTask> CellServerMsgToClientTaskPtr;
...
private:
	...
	//正式客户队列
	map<SOCKET, ClientSocketPtr> sock_pclient_pair;

	//缓冲客户队列
	vector<ClientSocketPtr> vec_client_buffer;
	//缓冲队列锁
	mutex m_mutex;
	//线程指针
	thread* m_pThread;
...
public:
	...
	//接收数据
	//处理粘包 拆分包
	int RecvData(ClientSocketPtr pClientSocket);

	//响应网络消息
	void OnNetMsg(ClientSocketPtr pclient_sock, DataHead* pHead);
	...
	void addSendTask(ClientSocketPtr pclient_sock, DataHeadPtr pHead)
	{
		//生产任务
		//CellServerMsgToClientTask* pCSMTCT = new CellServerMsgToClientTask(pclient_sock, pHead);

		CellServerMsgToClientTaskPtr pCSMTCT = make_shared<CellServerMsgToClientTask>(pclient_sock, pHead);
		...
		//对于智能指针，指向派生类的指针要赋给指向基类的指针，要进行强制类型转换
		m_CellTaskServer.addTask((CellTaskPtr&)pCSMTCT);
	}
};
...
private:
	SOCKET m_serv_sock;
	//vector<ClientSocket*> vec_client;
	vector<CellServerPtr> CellServers;
...
public:
	...
	void StartThread(int CellServerThreadsCnt)
	{
		...
		{
			auto serv = make_shared<CellServer>(m_serv_sock);
	...
	virtual void NEOnNetMsg(CellServer* pCellServer, ClientSocketPtr& client_sock, DataHead* pHead);
