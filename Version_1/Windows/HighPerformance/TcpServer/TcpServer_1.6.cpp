...
int ClientSocket::SendData(DataHeadPtr pHead)
{
	...
	//当前要发送的数据
	const char* pSendData = reinterpret_cast<const char*>(pHead.get());
...
int CellServer::RecvData(ClientSocketPtr pClientSocket)
{
...
void CellServer::OnNetMsg(ClientSocketPtr pclient_sock, DataHead* pHead)
{
	//++m_cnt;
...
SOCKET EasyTcpServer::Accept()
{
	...
	else
	{
		...
		*/
		...
		//***注***
		//只使用内存池，这样写能向内存池申请内存
		//addClientToCellServer(make_shared<ClientSocket>(client_sock));
		//但若要使用对象池，就要改变写法为
		ClientSocketPtr csp(new ClientSocket(client_sock));
		addClientToCellServer(csp);
	}
...
void EasyTcpServer::NEOnNetJoin(ClientSocketPtr& pClient)
{
	++m_clients_cnt;
}

//多线程(多CellServer)操作以下函数
void EasyTcpServer::NEOnNetLeave(ClientSocketPtr& pClient)
{
	--m_clients_cnt;
}

//多线程(多CellServer)操作以下函数
void EasyTcpServer::NEOnNetMsg(CellServer* pCellServer, ClientSocketPtr& pclient_sock, DataHead* pHead)
{
	++m_MsgCnt;
}

void EasyTcpServer::NERecv(ClientSocketPtr& client_sock)
{
	++m_RecvCnt;
}
#endif