...
public:
	...
	void* allocMemory(size_t size)
	{
		...
		if (nullptr == pHeader)
		{
			...
			//printf("allocMemory: id=%d, size=%d\n", pReturn->nID, size);
		}
		else
		{
			//使用pHeader指向的内存块
	...
	void initMemory()
	{
		xPrintf("initMemory:m_BlockSize=%d,m_BlockNums=%d\n", m_BlockSize, m_BlockNums);
		...
		pHeader = (MemoryBlockMsg*)m_pBuf;//解释为内存块描述信息
		pHeader->bPool = true;
		pHeader->nID = 0;
		pHeader->nRef = 0;
...
private:
	...
	MemoryAllocator<64, 1000000> m_mem64_10;
	MemoryAllocator<128, 1000000> m_mem128_10;
	MemoryAllocator<256, 100000> m_mem256_10;
	MemoryAllocator<512, 100000> m_mem512_10;
	MemoryAllocator<1024, 100000> m_mem1024_10;
